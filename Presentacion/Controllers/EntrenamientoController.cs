...
namespace FitRank_API.Controllers
{
    ...
    public class EntrenamientoController : ControllerBase
    {
        ...
        [HttpGet]
        public async Task<IActionResult> ObtenerTodosLosEntrenamientos() => Ok(await _obtenerTodos.Ejecutar());

        [HttpGet("{id}")]
        public async Task<IActionResult> ObtenerEntrenamientoPorId(long id)
        {
        ...
        [HttpPost]
        public async Task<IActionResult> Agregar([FromBody] AgregarEntrenamientoDTO dto)
        {
            var nuevo = await _crear.Ejecutar(dto);
            return CreatedAtAction(nameof(ObtenerEntrenamientoPorId), new { id = nuevo.Id }, nuevo);
        }
